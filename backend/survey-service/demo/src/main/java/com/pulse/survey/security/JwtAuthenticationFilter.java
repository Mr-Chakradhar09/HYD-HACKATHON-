    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (StringUtils.hasText(jwt) && tokenProvider.isTokenValid(jwt)) {
                ...
                String location = tokenProvider.extractLocation(jwt);

                if (userId == null) {
                    userId = username; // Fallback
                }

                UserPrincipal principal = UserPrincipal.create(userId, username, role, location);
                UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
                        principal, null, principal.getAuthorities());
                authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));

                SecurityContextHolder.getContext().setAuthentication(authentication);
                log.debug("Set Spring Security authentication context for user: {}", username);
            }
        } catch (Exception ex) {
